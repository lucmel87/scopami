...
import { GameAI } from './classes/GameAI.js';
...
class Scopa {
	...
	*/
	...
	async giocaCarta(giocatore, carta, cardElement) {
		...
		const possibiliPrese = this.gameAi.controllaPrese(carta, this.tavolo);
		let prese = null;
		if (possibiliPrese.length > 0) {
			...
			if (!giocatore.isCPU && possibiliPrese.length > 1) {
				...
				const bestMove = this.gameAi.getCPUBestMove(giocatore, carta, this.tavolo, this.mazzo, this.carteGiocate);
 ...
 */
	...
	cpuTurno(giocatoreCpu) {
		if (giocatoreCpu.mano.length > 0) {
			let bestMove = this.gameAi.getCPUBestMove(giocatoreCpu, null, this.tavolo, this.mazzo, this.carteGiocate); // la CPU non sa ancora che carta giocare, lascio NULL
	...
	calcolaPunteggio() {
		...
		(partitaFinita, giocatoreVincente);
	}
}
